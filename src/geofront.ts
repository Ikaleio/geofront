...
export class Connection {
	...
	get metrics(): ConnectionMetrics {
		const cached = this.geofront.getCachedConnectionMetrics(this._id)
		// 如果缓存中有数据（非零），更新并返回；否则返回最后已知的 metrics
		if (cached.bytes_sent > 0 || cached.bytes_recv > 0) {
...
export class Geofront {
	...
	static create(): Geofront {
		...
		try {
			...
			instance.enableMetricsPolling(1000)
		...
		try {
			...
			for (const [connId, connMetrics] of Object.entries(
				...
				this.connectionMetricsCache.set(Number(connId), {
					bytes_sent: connMetrics.bytes_sent,
					bytes_recv: connMetrics.bytes_recv
				})
			}
		} catch (error) {
	...
	*connections(): Generator<Connection> {
	...
	async shutdown() {
		...
		// Clear connection state to prevent interference between tests
		activeConnections.clear()
	...
	private enableMetricsPolling(intervalMs: number = 1000) {
	...
	private pollBatchEvents() {
		...
		try {
			...
			console.error('[ERROR] Error polling batch events:', e)
		} finally {
			if (eventsPtr) {
