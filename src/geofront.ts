...
// ===== 核心函数类型 =====
export type RouterFn = (context: RouteContext) => RouteResult | Promise<RouteResult>
...
export type ConnectionClosedHandler = (connection: Connection, info: ConnectionInfo) => void
...
export class Connection {
	...
	setRateLimit(limit: RateLimit): void {
		...
		this.proxy.setRateLimit(this.id, uploadAvg, uploadBurst, downloadAvg, downloadBurst)
	}
...
export class GeofrontProxy {
	...
	private initializeFFI() {
		try {
			...
			if (isDev) {
				console.warn(`[WARN] Development mode: Loading FFI library from: ${libPath}`)
			}
	...
	getConnectionsByPlayer(player: string): ReadonlyArray<Connection> {
		return Array.from(this.connections.values()).filter(conn => conn.player === player)
	}
	...
	getConnectionsByHost(host: string): ReadonlyArray<Connection> {
		return Array.from(this.connections.values()).filter(conn => conn.host === host)
	}
	...
	getPlayerCount(): number {
		const players = new Set(Array.from(this.connections.values()).map(conn => conn.player))
		return players.size
	}
	...
	updateMetrics(): void {
		try {
			...
			for (const [connId, connMetrics] of Object.entries(rawMetrics.connections || {})) {
			...
			if (this.eventHandlers.onError) {
				this.eventHandlers.onError(new Error(`Failed to update metrics: ${error}`))
			}
		}
	}
	...
	private pollBatchEvents(): void {
		...
		try {
			...
			if (this.eventHandlers.onError) {
				this.eventHandlers.onError(new Error(`Error polling batch events: ${e}`))
			}
		} finally {
			if (eventsPtr) {
...
export namespace Geofront {
	...
	export function staticMotd(motd: {
		...
		players: { max: number; sample?: ReadonlyArray<{ name: string; id: string }> }
