...
// ===== 模拟 SOCKS5 代理服务器 =====
function startSocksServer(): Promise<{
	server: import('socksv5').SocksServer
	closed: Promise<void>
}> {
	return new Promise((resolve, reject) => {
		const server = socks.createServer((info, accept, deny) => {
			console.log(`[SOCKS5] 代理请求: ${info.dstAddr}:${info.dstPort}`)
			accept()
		})

		const closedPromise = new Promise<void>(resolveClosed => {
			server.on('error', err => {
				console.error(`[SOCKS5] 服务器错误: ${err}`)
			})
			// `close` event is not explicitly handled as we control the lifecycle
		})

		server.listen(SOCKS_PORT, '127.0.0.1', () => {
			console.log(`[SOCKS5] 代理服务器已在 127.0.0.1:${SOCKS_PORT} 上启动`)
...
// ===== 主测试函数 =====
async function main() {
	console.log('=== 开始 Geofront SOCKS5 代理端到端测试 ===')
	let geofront: Geofront | null = null
	let backendServer: import('net').Server | null = null
	let socksServer: import('socksv5').SocksServer | null = null

	try {
		// 1. 启动 SOCKS5 代理服务器
		const { server: srv, closed: socksPromise } = await startSocksServer()
		socksServer = srv
		...
		console.log('\n✅✅✅ Geofront SOCKS5 代理端到端测试成功! ✅✅✅')
	} catch (error) {
		console.error('\n❌❌❌ Geofront SOCKS5 代理端到端测试失败! ❌❌❌')
		console.error(error)
		process.exit(1)
	} finally {
		// 8. 清理
